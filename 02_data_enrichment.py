...
# MAGIC
# MAGIC Now, we can enrich the pDNS data with tldextract, GeoIP lookups, a DGA Classifier, URLHaus, threat intel lookups. We will do this using Spark SQL.
...
display(dns_table
        .withColumn("geoip_data", get_geoip_data(dns_table.ip_address))
        .selectExpr("*", "geoip_data.*", "case when char_length(domain_extract(rrname)) > 5 then ioc_detect(string(domain_extract(rrname))) else null end as ioc"))
...
display(dns_table.selectExpr("ioc_detect(string(domain_extract(rrname)))"))

# COMMAND ----------

display(dns_table_enriched)
